...
tech-atrium-wiki（通称: techrium）は、開発部全員参加型の技術 Wiki / 学習ポータルである。Next.js（SSG/SSR 混合）+ React を Frontend に採用し、デプロイ・インフラ方式は以下の 3 案から選択する（Supabase / Vercel は候補案 A であり、唯一の前提ではない）。

初学者・中級者・上級者の 3 階層ユーザーに対し、MDX ベースの多層的コンテンツ、動的比較エンジン、GitHub OAuth 認証、コメント機能、プルリク型修正提案フローを提供する。Techrium Theme（プリズム・虹色グラデーション）を全体に適用し、視覚的に直感的な UI を実現する。メイン開発者が React フロントと統合方針を主導し、共同開発者は `stack/` 配下で技術スタック別の学習サンプル・PoC・検証コードを持ち寄る。

---

## アーキテクチャ

### 案 A: Vercel + Supabase（最短 MVP）
...
graph TD
    ...
    subgraph Supabase["Backend: Supabase（案A）"]
        AUTH[Supabase Auth<br/>GoTrue]
        DB[(Supabase PostgreSQL<br/>データストア)]
...
graph TD
    ...
    subgraph CloudRun["App: Cloud Run（案B）"]
        direction TB
        NEXT[Next.js standalone<br/>コンテナ]
        MDX[MDX Content<br/>Git管理・ビルド時生成]
        RC[React Components<br/>動的UI]
    end

    subgraph Auth["Auth: Auth.js / NextAuth"]
        AUTHJS[Auth.js<br/>GitHub OAuth]
    end

    subgraph Neon["Database: Neon PostgreSQL"]
        DB[(Neon PostgreSQL<br/>データストア)]
    end

    subgraph Storage["Storage（初期）"]
        GCS[GCS or Git管理<br/>画像・アセット]
    end
    ...
    NEXT --> RC
    RC -->|セッション確認| AUTHJS
    AUTHJS -->|OAuth フロー| OAUTH
    OAUTH -->|コールバック| AUTHJS
    RC -->|データ取得/更新| DB
    NEXT -->|画像参照| GCS
```

### 案 C-1: 全部 VPS + Docker Compose
...
graph TD
    ...
    subgraph VPS["VPS（案C-1）"]
        direction TB
        subgraph Proxy["リバースプロキシ"]
            CADDY[Caddy or Nginx<br/>TLS終端・ルーティング]
        end
        subgraph App["アプリケーション"]
            NEXT[Next.js standalone<br/>コンテナ]
        end
        subgraph DB["データベース"]
            PG[(PostgreSQL<br/>コンテナ)]
        end
    end
...
> **案 C-2（Cloud Run + VPS DB）について**: Cloud Run から VPS 上の PostgreSQL への接続は、外部公開とネットワーク・セキュリティ設計（ファイアウォール、TLS、IP 許可リスト等）が必要であり、初期 MVP の第一候補としては推奨しない。
...
| UI Library | React 18.2 | 動的インタラクティブコンポーネント |
| Styling | Tailwind CSS | Techrium Theme 実装 |
| Content | MDX（Git 管理） | 記事コンテンツ管理 |
...
| レイヤー | 案 A | 案 B | 案 C-1 |
|---|---|---|---|
| Auth | Supabase Auth (GoTrue) | Auth.js + GitHub OAuth | Auth.js + GitHub OAuth |
| Database | Supabase PostgreSQL | Neon PostgreSQL | VPS PostgreSQL |
| Storage | Supabase Storage | GCS または Git 管理 | VPS ローカルまたは GCS |
| Hosting | Vercel | Google Cloud Run | VPS (Caddy/Nginx) |
...
├── stack/            # 学習・検証・PoC（本番 API 群ではない）
│   ├── language/
│   ├── framework/
│   └── database/
├── app/              # メインフロントのページ
└── lib/api/          # API 呼び出し層・型定義
```

### 運用前提

- フロントエンド実装はメイン開発者が `React / Next.js` 前提で統合する
- 共同開発者は `stack/` 配下で担当技術スタックの学習サンプル・PoC・検証コードを管理する（本番統合は自動ではない）
- `stack/framework/react/` はフロント統合観点の共通契約置き場として扱う
- `stack/database/` は DB 学習・設計知見・接続サンプルの置き場として扱う
...
MDX 記事は Git 管理であり、デプロイ方式に依存しない。以下は案 A（Vercel）の例だが、案 B・C でも同様の流れになる。

```mermaid
sequenceDiagram
    participant Dev as 開発者
    participant Git as Git Repository
    participant Build as ビルド環境
    participant Next as Next.js SSG
    participant MDX as MDX Processor
    participant Edge as 配信サーバー
...
### 動的データのランタイムフェッチフロー（案 A 例: Supabase）
...
sequenceDiagram
    ...
    Next->>Auth: OAuth 認証開始
    Auth->>GitHub: OAuth 認証リクエスト
    GitHub-->>User: 認証画面表示
    User->>GitHub: 認証承認
    GitHub->>Auth: コールバック（code）
    Auth->>GitHub: アクセストークン取得
    GitHub-->>Auth: ユーザー情報返却
    Auth->>DB: USERS テーブルへ upsert
    Auth-->>Next: セッション（JWT）発行
    Next->>Next: Cookie にセッション保存
    Next-->>User: ログイン完了・リダイレクト
```

> **案 B の場合**: `Auth Service` が Auth.js に変わる。その他のフローは同様。
...
| DB 接続失敗 | 全サービス | エラーログ記録 + サービス一時停止メッセージ表示 |
...
- DB クライアントのエラーは `lib/db.ts`（または `lib/supabase.ts`）で一元的にハンドリングし、型安全なエラーオブジェクトを返す
